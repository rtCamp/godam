...
const VideoThumbnails = ( { handleSettingChange } ) => {
	...
	const [ videoThumbnailsInput, setVideoThumbnailsInput ] = useState( String( videoThumbnails ) );

	useEffect( () => {
		setVideoThumbnailsInput( String( videoThumbnails ) );
	}, [ videoThumbnails ] );

	const handleVideoThumbnailsInputChange = ( value ) => {
		if ( /^\d{0,2}$/.test( value ) ) {
			setVideoThumbnailsInput( value );
		}
	};
			...
			<PanelBody>
				<div className="godam-form-group">
					...
					<TextControl
						...
						min="1"
						max="10"
						value={ videoThumbnailsInput }
						onChange={ ( value ) => handleVideoThumbnailsInputChange( value ) }
						onBlur={ validateVideoThumbnails }

			...
			<div className="easydam-settings-header border-b -ml-[32px] pl-[32px] bg-white">
				<div className="max-w-[1440px] mx-auto pl-4 pr-9 flex items-center justify-between">
					...
					<div className="flex items-center">
						...
						<Button
							className={ `ml-2 godam-button ${ ( ! window?.userData?.validApiKey || ! window?.userData?.user_data?.active_plan ) ? 'disabled' : '' }` }
							variant="primary"
							size="compact"
							target={ ( window?.userData?.validApiKey && window?.userData?.user_data?.active_plan ) ? '_blank' : undefined }
							text={ __( 'Manage Media', 'godam' ) }
							href={ ( window?.userData?.validApiKey && window?.userData?.user_data?.active_plan ) ? godamMediaLink : '#' }
							icon={
							...
							label={ ( ! window?.userData?.validApiKey || ! window?.userData?.user_data?.active_plan ) ? __( 'Premium feature', 'godam' ) : __( 'GoDAM central media manager', 'godam' ) }
							tooltipText={ __( 'Manage Media', 'godam' ) }
							// disabled={ ! window?.userData?.validApiKey || ! window?.userData?.user_data?.active_plan }
						/>

						{
							( window?.userData?.validApiKey && window?.userData?.user_data?.active_plan && ( window?.userData?.user_data?.active_plan )?.toLowerCase() !== 'platinum' ) &&
							<Button
						...
						{
							( ! window?.userData?.validApiKey || ! window?.userData?.user_data?.active_plan ) &&
								<Button
									className="ml-2 godam-button"
									variant="primary"
									size="compact"

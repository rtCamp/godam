 ...
 *
 * In active use by SingleLayerAnalyticsList. The tab-switch clearing
 * bug (stale bars persisting when switching to an empty-data tab)
 * is now fixed at the call site — the useEffect always clears the
 * container before any data check.
 *
 * Three smaller issues remain as v1.5 polish — none block shipping.
 * (1) Sparse data (1-2 days) renders awkwardly in a 350px-tall chart;
 * pad the X axis to the full range with zero bars OR hide below a
 * threshold. (2) Y-axis tick formatting on small integer ranges can
 * produce duplicate labels ("3, 2, 1, 1, 0"); use .ticks(Math.min(5, yMax))
 * with d3.format('d') to constrain. (3) Color palette pink (#AB3A6C)
 * for the "submitted" / "voted" series visually competes with the
 * active-tab pill background — switch to the dashboard's standard
 * analytics palette.
 *
 ...
 * stays readable; everything else uses daily buckets.
 *
 * The chart is intentionally simple — no tooltips, no axes labels beyond
 * the rotated date strings. The right-hand summary cards (rendered by
 * SingleLayerAnalyticsList) carry the precise totals.
 *
 * @param {Array<Object>} data        daily_breakdown rows from /processed-layer-analytics/. Each row has a `date` and one numeric field per action_type the layer type emits.
 * @param {string}        containerId DOM selector for the host element.
 * @param {string}        duration    '7d' | '30d' | '60d' | '1y'.
 * @param {string[]}      subgroups   Metric keys to render as bars per bucket.
 * @return {void}
 */
export function layerAnalyticsBarChart( data, containerId, duration, subgroups ) {
	const margin = { top: 20, right: 16, bottom: 60, left: 36 };
	const containerEl = document.querySelector( containerId );
	if ( ! containerEl ) {
		return;
	}

	// Clear any previous render so re-renders don't stack SVGs.
	...
	// For 1-year, aggregate by month so the bar count is manageable.
	let chartData = data;
	if ( duration === '1y' ) {
		const monthly = d3.rollup(
			...
			( d ) => String( d.date ).slice( 0, 7 ),
		);
		chartData = Array.from( monthly, ( [ date, vals ] ) => ( {
			date,
			...vals,
		} ) );
	}
	...
	const yMax =
		d3.max( chartData, ( d ) => d3.max( subgroups, ( key ) => d[ key ] || 0 ) ) || 1;
	...
	// Color scheme that mirrors the rest of the dashboard palette.
	const color = d3
		.scaleOrdinal()
		.domain( subgroups )
		.range( [ '#AB3A6C', '#62C2B3', '#3775D1', '#F59E0B' ] );

	// X axis — rotate ticks 30deg for readability on dense ranges.
	svg
		.append( 'g' )
	...
	// Y axis — integer ticks since counts are whole numbers.
	svg
		.append( 'g' )
	...
	svg
		...
		.selectAll( 'rect' )
		.data( ( d ) => subgroups.map( ( key ) => ( { key, value: d[ key ] || 0 } ) ) )
		.join( 'rect' )
		.attr( 'x', ( d ) => x1( d.key ) )
		.attr( 'y', ( d ) => y( d.value ) )
	...
	// Legend across the top of the chart area.
	const legend = svg
		.append( 'g' )
		.attr( 'transform', `translate(0,${ -10 })` );
	subgroups.forEach( ( key, i ) => {
		const item = legend.append( 'g' ).attr( 'transform', `translate(${ i * 70 },0)` );
		item
			.append( 'rect' )
			.attr( 'width', 10 )
			.attr( 'height', 10 )

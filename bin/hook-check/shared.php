 ...
 *
 * register_activation_hook()/register_deactivation_hook() take __FILE__
 * (not a string) as their first argument, so hook_name comes back null for
 * those. register_rest_route()'s callback, nested in its 'callback' => ...
 * entry, is already handled since the search scans every token in the
 * call's parens regardless of argument position.
 *
 * @var string[]
 */

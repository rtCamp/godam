<?php
/**
 * Shared render template for the GoDAM Video Gallery V2.
 *
 * Used by:
 *   - assets/src/blocks/godam-gallery-v2/render.php    (Gutenberg block)
 *   - inc/classes/shortcodes/class-godam-video-gallery.php  (shortcode)
 *   - inc/classes/rest-api/class-dynamic-gallery.php   (only require_once'd for
 *     access to the helper functions below; never renders)
 *
 * Required call-site variables to render:
 *   $attributes              array  Block-shaped attributes (camelCase keys).
 *                                    When NOT set, the file only registers its
 *                                    helper functions and returns without output.
 *   $is_shortcode            bool   Optional; when true, build wrapper attributes
 *                                    manually instead of calling
 *                                    get_block_wrapper_attributes() (which only
 *                                    works inside a block render context).
 *   $inner_block_video_ids   int[]  Optional; pre-resolved video attachment IDs
...
// Helper functions
//
// Declared at the top of the template so callers that only need the helpers
// (e.g. the gallery-shortcode REST endpoint) can `require_once` this file
// without triggering a render. The render section below is gated on
// $attributes being set.
// -----------------------------------------------------------------------------

if ( ! function_exists( 'rtgodam_gallery_v2_parse_id_list' ) ) {
	 ...
	 * @return int[]
	 */
	function rtgodam_gallery_v2_parse_id_list( $value ) {
...
if ( ! function_exists( 'rtgodam_gallery_v2_get_thumbnail_url' ) ) {
	 ...
	 * @return string
	 */
	function rtgodam_gallery_v2_get_thumbnail_url( $attachment_id ) {
...
if ( ! function_exists( 'rtgodam_gallery_v2_get_placeholder_thumbnail_url' ) ) {
	 ...
	 * @return string
	 */
	function rtgodam_gallery_v2_get_placeholder_thumbnail_url( $attachment_id ) {
		$thumbnail_sources = rtgodam_get_video_thumbnail_sources( $attachment_id );
...
if ( ! function_exists( 'rtgodam_gallery_v2_format_display_date' ) ) {
	 ...
	 * @return string
	 */
	function rtgodam_gallery_v2_format_display_date( $date_string ) {
...
if ( ! function_exists( 'rtgodam_gallery_v2_get_relative_date' ) ) {
	 ...
	 * @return string
	 */
	function rtgodam_gallery_v2_get_relative_date( $days ) {
...
if ( ! function_exists( 'rtgodam_gallery_v2_build_query_args' ) ) {
	 ...
	 * @return array
	 */
	function rtgodam_gallery_v2_build_query_args( $attributes, $page = 1 ) {
		$media_folder_ids = rtgodam_gallery_v2_parse_id_list( $attributes['mediaFolder'] ?? '' );
		$author_ids       = rtgodam_gallery_v2_parse_id_list( $attributes['author'] ?? '' );
		...
		if ( '7days' === $date_range ) {
			$date_query[] = array( 'after' => rtgodam_gallery_v2_get_relative_date( 7 ) );
		} elseif ( '30days' === $date_range ) {
			$date_query[] = array( 'after' => rtgodam_gallery_v2_get_relative_date( 30 ) );
		} elseif ( '90days' === $date_range ) {
			$date_query[] = array( 'after' => rtgodam_gallery_v2_get_relative_date( 90 ) );
...
if ( ! function_exists( 'rtgodam_gallery_v2_get_video_data' ) ) {
...
// Render
//
// Skipped when $attributes is not set so the file is safe to require_once
...
$godam_is_shortcode = isset( $is_shortcode ) && $is_shortcode;
...
if ( $godam_is_shortcode ) {
	// get_block_wrapper_attributes() is only meaningful inside a block render
	// context; in the shortcode path we build the same attribute set manually.
	$godam_wrapper_parts = array();
	foreach ( $godam_wrapper_attrs as $godam_attr_key => $godam_attr_value ) {
		$godam_wrapper_parts[] = sprintf( '%s="%s"', $godam_attr_key, esc_attr( (string) $godam_attr_value ) );
	}
	$godam_wrapper_attributes = implode( ' ', $godam_wrapper_parts );
} else {
	$godam_wrapper_attributes = get_block_wrapper_attributes( $godam_wrapper_attrs );
}
...
if ( 'query' === $godam_gallery_mode ) {
	...
	if ( $godam_query->have_posts() ) {
		foreach ( $godam_query->posts as $godam_video_post ) {
			$godam_item = rtgodam_gallery_v2_get_video_data( $godam_video_post->ID );
	...
	foreach ( $inner_block_video_ids as $godam_video_id ) {
		$godam_item = rtgodam_gallery_v2_get_video_data( $godam_video_id );

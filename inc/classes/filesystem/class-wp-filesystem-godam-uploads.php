...
class WP_Filesystem_GoDAM_Uploads extends \WP_Filesystem_Base {

	/** @var API_Client */
	private $api;

	public function __construct( Api_Client $api_client ) {
		$this->method = 'godam-uploads';
	 ...
	 *
	 * This method should not be called directly and instead should be triggered via `WP_Filesystem_GoDAM`.
	 *
	...
	public function copy( $source, $destination, $overwrite = false, $mode = false ) {
		...
		$error_msg = sprintf( __( 'The `%s` method cannot be called directly. Please use `WP_Filesystem_GoDAM::copy` instead' ), __METHOD__ );
	...
	public function move( $source, $destination, $overwrite = false ) {
		// translators: 1 - method name
		$error_msg = sprintf( __( 'The `%s` method cannot be called directly. Please use `WP_Filesystem_GoDAM::move` instead' ), __METHOD__ );

 ...
 */
// Note: we're using `PHP_INT_MAX` for the priority because we want our `WP_Filesystem_GoDAM` class to always take precedence.

define( 'VIP_FILESYSTEM_METHOD', 'GoDAM' );
...
require_once __DIR__ . '/class-wp-filesystem-godam.php';
...
class WP_Filesystem_GoDAM extends RTGODAM\Inc\Filesystem\WP_Filesystem_GoDAM {}
...
add_filter(
	...
	function ( $credentials, $form_post, $type ) {
		...
		if ( '' === $type || VIP_FILESYSTEM_METHOD === $type ) {
			if ( true === WPCOM_IS_VIP_ENV ) {
				$api_client  = RTGODAM\Inc\Filesystem\new_api_client();
				$credentials = array(
					new RTGODAM\Inc\Filesystem\WP_Filesystem_GoDAM_Uploads( $api_client ),
...
add_filter(
	...
	function ( $file, $method ) {
		if ( 'GoDAM' === $method ) {
			$file = __DIR__ . '/class-wp-filesystem-godam.php';
		}
		return $file;
	},
	PHP_INT_MAX,

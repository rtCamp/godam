...
class GoDAM_Filesystem_Local_Stream_Wrapper {
		...
		try {
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( $this->should_flush_empty ) {
					 ...
					 */
					$this->client->cache_file_stats( $this->path, array() );
				}
	...
	private static function get_local_tmp_path( $file_path ) {
		...
		if ( preg_match( '#(wp-content/.+)$#', $file_path, $matches ) ) {
			...
			// Handle godam:// paths
			$relative_path = substr( $file_path, 8 ); // Remove 'godam://' prefix
		} else {

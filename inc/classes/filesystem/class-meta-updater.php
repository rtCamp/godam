...
class Meta_Updater {
	...
	private function update_attachment_filesize( $attachment_id ): array {
		...
		if ( ! is_array( $meta ) ) {
			return array( 'skip-invalid-metadata', 'does not have valid metadata' );
		}

		if ( isset( $meta['filesize'] ) ) {
			return array( 'skip-has-filesize', 'already has filesize' );
		}

		$filesize = $this->get_filesize_from_file( $attachment_id );

		if ( $filesize < 0 ) {
			return array( 'fail-get-filesize', 'failed to get filesize' );
		}

		$meta['filesize'] = $filesize;

		if ( $this->dry_run ) {
			return array( 'skip-dry-run', 'dry-run; would have updated filesize to ' . $filesize );
		}

		wp_update_attachment_metadata( $attachment_id, $meta );
		return array( 'updated', 'updated filesize to ' . $filesize );
	}

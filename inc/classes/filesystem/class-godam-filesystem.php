<?php
/**
 * GoDAM Filesystem Class
 *
 * This class handles the integration with the GoDAM Filesystem API, providing methods to manage files
 * and directories within the GoDAM system.
 *
...
class GoDAM_Filesystem {
	...
	public function __construct() {
		...
		$this->plugin_name = 'godam-filesystem';
	...
	private function load_dependencies() {
		 ...
		 */
		require_once __DIR__ . '/class-godam-filesystem-local-stream-wrapper.php';
	}
	...
	public function run() {
		...
		$this->stream_wrapper = new GoDAM_Filesystem_Local_Stream_Wrapper(
			new_api_client(),
			self::PROTOCOL
		);
	...
	private function add_filters() {
		...
		add_filter( 'get_attached_file', array( $this, 'filter_get_attached_file' ), 20 );
		add_filter( 'wp_generate_attachment_metadata', array( $this, 'filter_wp_generate_attachment_metadata' ), 10, 2 );
		add_filter( 'wp_read_image_metadata', array( $this, 'filter_wp_read_image_metadata' ), 10, 2 );
	...
	private function remove_filters() {
		remove_filter( 'upload_dir', array( $this, 'filter_upload_dir' ), 10 );
		...
		remove_filter( 'wp_delete_file', array( $this, 'filter_delete_file' ), 20 );
		remove_filter( 'get_attached_file', array( $this, 'filter_get_attached_file' ), 20 );
		remove_filter( 'wp_generate_attachment_metadata', array( $this, 'filter_wp_generate_attachment_metadata' ) );
		remove_filter( 'wp_read_image_metadata', array( $this, 'filter_wp_read_image_metadata' ), 10, 2 );
	 ...
	 * Return uploads path
	 *
	 * Different from wp_uploads_dir() as that will return the `godam://` protocol too
	 *
	 * @since   1.0.0
	 * @access  private
	 *
	 ...
	 *
	 * Strip query string and `godam` protocol to avoid attempting to delete the aforementioned image sizes
	 *
	 * @since   1.0.0
	 * @access  private
	 *
	...
	private function purge_file_cache( $file_uri ) {
		...
		if ( ! \WPCOM_VIP_Cache_Manager::instance()->queue_purge_url( $invalidation_url ) ) {
			...
			trigger_error(
				...
				sprintf( __( 'Error purging %s from the cache service #godam-streams' ), $invalidation_url ),
				E_USER_WARNING
			);
			// phpcs:enable
		}
	...
	public function filter_wp_read_image_metadata( $meta, $file ) {
		...
		remove_filter( 'wp_read_image_metadata', array( $this, 'filter_wp_read_image_metadata' ), 10 );

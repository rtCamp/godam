<?php
/**
 * Stream Wrapper plugin for GoDAM
 *
 * @package RTGODAM\Inc
 */
...
if ( ! defined( 'FILE_SERVICE_URI' ) ) {
	define( 'FILE_SERVICE_URI', 'http://127.0.0.1:8000' );
}
...
require_once __DIR__ . '/filesystem/class-path-utils.php';

require_once __DIR__ . '/filesystem/init-filesystem.php';

require_once __DIR__ . '/filesystem/class-godam-filesystem.php';

require_once __DIR__ . '/filesystem/acl/acl.php';

/**
 * The class use to update attachment meta data
 */
require_once __DIR__ . '/filesystem/class-meta-updater.php';


// use Automattic\Jetpack\Image_CDN\Image_CDN;
use RTGODAM\Inc\Filesystem\GoDAM_Filesystem;
use RTGODAM\Inc\Filesystem\Meta_Updater;
...
class Stream_Wrapper {
	...
	private function init_vip_filesystem() {
		$vip_filesystem = new GoDAM_Filesystem();
		$vip_filesystem->run();
	}
	...
	private function init_jetpack_photon_filters() {
		...
		add_filter( 'jetpack_photon_domain', array( 'A8C_Files_Utils', 'filter_photon_domain' ), 10, 2 );
		...
		add_filter(
			'the_content',
			function ( $content ) {
				add_filter( 'jetpack_photon_pre_image_url', array( 'A8C_Files_Utils', 'strip_dimensions_from_url_path' ) );
				return $content;
			},
			0
		);
		...
		add_filter(
			'the_content',
			function ( $content ) {
				remove_filter( 'jetpack_photon_pre_image_url', array( 'A8C_Files_Utils', 'strip_dimensions_from_url_path' ) );
				return $content;
			},
			9999999
		); // Jetpack hooks in at 6 9s (999999) so we do 7
	...
	public function filter_cron_schedules( $schedule ) {
		...
		$schedule['vip_five_minutes'] = array(
			'interval' => 180,
	...
	public function schedule_update_job() {
		...
		add_action( self::CRON_EVENT_NAME, array( $this, 'update_attachment_meta' ) );
	}
	...
	public function update_attachment_meta() {
		Alerts::chat(
			...
			sprintf(
				'Starting %s on %s... $vip-go-streams-debug',
		...
		if ( get_option( self::OPT_ALL_FILESIZE_PROCESSED ) ) {
			...
			Alerts::chat(
				'#vip-go-filesize-updates',
				sprintf(
					'Already completed updates on %s. Exiting %s... $vip-go-streams-debug',
					home_url(),
					self::CRON_EVENT_NAME
				),
				5
		...
		do {
			if ( $start_index > $max_id ) {
				...
				Alerts::chat(
					'#vip-go-filesize-updates',
					sprintf(
						'Passed max ID (%d) on %s. Exiting %s... $vip-go-streams-debug',
						$max_id,
						home_url(),
						self::CRON_EVENT_NAME
					),
		...
		Alerts::chat(
			'#vip-go-filesize-updates',
			sprintf(
				'Batch %d to %d (of %d) completed on %s. Processed %d attachments (%s) $vip-go-streams-debug',
				$orig_start_index,
				$start_index,
				$max_id,
				home_url(),

	...
	return (
		<div className="p-2 w-full border rounded">
			<div className="flex justify-between items-center">
				...
				<DropdownMenu
					icon={ moreVertical }
					label={ `${ getProductHotspotDisplayName( productHotspot, index, productData ) } ${ __( 'options', 'godam-woo' ) }` }
					toggleProps={ {
						'aria-label': sprintf(
							/* translators: %s: Product hotspot display name. */
							__( 'Options for %s', 'godam-woo' ),
							getProductHotspotDisplayName( productHotspot, index, productData ),
						),
					} }
				>
					{ () => (
						<>
							...
							>
								{ __( 'Delete Product Hotspot', 'godam-woo' ) }
							</MenuItem>
						</>
					) }
				</DropdownMenu>
				...
				<div className="mt-3">
					...
					<div className="mt-3">
						{ ( () => {
							...
							if ( isGrouped ) {
								help = __( 'Grouped products cannot be added to the cart directly.', 'godam-woo' );
							} else if ( isExternal ) {
								help = __( 'External/Affiliate products cannot be added to the cart directly.', 'godam-woo' );
							} else if ( isVariable ) {
								help = __( 'Variable products cannot be added to the cart directly.', 'godam-woo' );
							} else if ( isOutOfStock ) {
								help = __( 'Products Out of Stock cannot be added to the cart directly.', 'godam-woo' );
							} else if ( productHotspot.addToCart ) {
								help = __( 'Users will be redirected to the product page.', 'godam-woo' );
							} else {
								help = __( 'By default, this adds the product to the cart. Turn ON to go to the product page instead.', 'godam-woo' );
							}

							return (
								<ToggleControl
									className="godam-toggle"
									label={ __( 'Link to Product Page', 'godam-woo' ) }
		...
		<div className={ `hotspot-content flex items-center justify-center ${ ! ( productHotspot.icon || productHotspot.customIconUrl ) ? 'no-icon' : '' }` }>
			...
			<div className="product-hotspot-box">
				{ productData ? (
					...
					<div>{ __( 'No product selected', 'godam-woo' ) }</div>
				) }
			</div>
		...
		<>
...

			{
				...
				<Notice
					...
					isDismissible={ false }
				>
					{ __( 'This features is available in premium version', 'godam-woo' ) }
				</Notice>
			}
			...
			<div className="mb-4">
				<TextControl
					label={ __( 'Layer Duration (seconds)', 'godam-woo' ) }
					...
					help={ __( 'Duration (in seconds) this layer will stay visible', 'godam-woo' ) }
					disabled={ ! isValidAPIKey }
				/>
			</div>
			...
			<div className="mb-4">
				<ToggleControl
					className="godam-toggle"
					label={ __( 'Pause video on hover', 'godam-woo' ) }
			...
			<div className="mb-4">
				<ToggleControl
					className="godam-toggle"
					label={ __( 'Show/Hide Mini Cart', 'godam-woo' ) }
					checked={
			...
			<div className="flex items-center flex-col gap-4 pb-4">
				...
				<Button
					...
					disabled={ ! isValidAPIKey }
				>
					{ __( 'Add Product Hotspot', 'godam-woo' ) }
				</Button>
			</div>
			...
			<LayerControls>
				{ layer?.miniCart && (
					...
					>
						<Tooltip
							text={ __( 'This is just a preview for Mini Cart. It will change styles according to your theme.', 'godam-woo' ) }
							placement="top-start"
						>
							<div
								className="

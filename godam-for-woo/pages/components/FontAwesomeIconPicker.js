...
const FontAwesomeIconPicker = ( { productHotspot, disabled = false, index, productHotspots, updateField } ) => {
	...
	const handleUploadCustomIcon = () => {
		const fileFrame = wp.media( {
			title: __( 'Select or Upload Custom Icon', 'godam-woo' ),
			button: {
				text: __( 'Use this icon', 'godam-woo' ),
			},
			library: {
		...
		fileFrame.on( 'select', function() {
			...
			if ( attachment.type !== 'image' ) {
				showNotice( __( 'Only Image file is allowed', 'godam-woo' ), 'error' );
				return;
			}
	...
	return (
		<div className="flex flex-col gap-2 mt-3">
			...
			>
				{ __( 'PRODUCT HOTSPOT ICON', 'godam-woo' ) }
			</label>
					...
					renderToggle={ ( { isDropDownOpen, onToggle } ) => (
						...
						>
							{ productHotspot.icon ? (
								...
								<span>{ __( 'Select Icon', 'godam-woo' ) }</span>
							) }
						</Button>
					) }
					renderContent={ () => (
						...
						>
							<TextControl
								placeholder={ __( 'Search icons…', 'godam-woo' ) }
								value={ searchQuery }
				...
				>
					{ productHotspot.customIconUrl ? (
						<>
							...
							/>
							<span>{ __( 'Custom Icon', 'godam-woo' ) }</span>
						</>
					) : (
						<span>{ __( 'Custom Icon', 'godam-woo' ) }</span>
					) }

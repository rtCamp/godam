...
export default class WooCommerceLayerManager {
	...
	// Layer registry interface methods — called by layersManager for custom layers.
	setupLayer( layer, layerElement ) {
		this.setupWooCommerceLayer( layer, layerElement );
	}

	handleTimeUpdate( currentTime ) {
		this.handleWooCommerceLayersTimeUpdate( currentTime );
	}
	...
	showFullscreenReturnMessage() {
		...
		const message = __( 'Close cart & return to fullscreen', 'godam-woo' );
		const returnButton = __( 'Return to Fullscreen', 'godam-woo' );
	...
	createCustomIcon( customIconUrl, backgroundColor, hotspotDiv ) {
		...
		customIconEl.alt = __( 'Custom Icon', 'godam-woo' );
	...
	createProductHotspotProductBox( hotspot ) {
		...
		productBoxDiv.style.display = 'flex !important';
		...
		if ( hotspot?.productId ) {
			...
			placeholderDiv.textContent = __( 'No product found', 'godam-woo' );
		}
		...
		const isOutOfStock = hotspot.productDetails?.in_stock === false;

		const forceProductPage =
			isVariable || isGrouped || isExternal || isOutOfStock;
		...
		productLinkButton.addEventListener( 'click', ( event ) => {
			...
			if ( hotspot.addToCart ) {
				...
				if ( ! cartStore || typeof cartStore.addItemToCart !== 'function' ) {
					...
					productLinkButton.disabled = false;
					productLinkButton.classList.remove( 'loading' );
					this.showCartMessage( __( 'WooCommerce cart is not available. Please refresh the page.', 'godam-woo' ), 'error' );
					return;
				}
				...
				cartStore
					...
					.then( () => {
						productLinkButton.disabled = false;
						productLinkButton.classList.remove( 'loading' );
						this.showCartMessage( __( 'Product added successfully!', 'godam-woo' ), 'success' );
					} )
					.catch( ( err ) => {
						// eslint-disable-next-line no-console
						...
						if ( err?.code === 'woocommerce_rest_product_partially_out_of_stock' ) {
							this.showCartMessage( __( 'Product is partially out of stock.', 'godam-woo' ), 'error' );
						} else if ( err?.code === 'woocommerce_rest_product_out_of_stock' ) {
							this.showCartMessage( __( 'Product is out of stock.', 'godam-woo' ), 'error' );
						} else {
							this.showCartMessage( __( 'Something went wrong. Try again.', 'godam-woo' ), 'error' );
						}
					} );
			}
		} );

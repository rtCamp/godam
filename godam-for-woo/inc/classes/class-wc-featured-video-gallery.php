...
class WC_Featured_Video_Gallery {
	...
	public function enqueue_admin_scripts() {
		...
		wp_enqueue_script(
			'godam-woo-admin-featured-video-gallery',
			GODAM_WOO_URL . 'assets/build/js/admin/wc-admin-featured-video-gallery.min.js',
			array( 'jquery', 'media-editor', 'media-views', 'wp-i18n' ),
			godam_woo_get_asset_version( GODAM_WOO_ASSETS_BUILD_PATH . 'js/admin/wc-admin-featured-video-gallery.min.js' ),
			true
		);
	...
	public function register_frontend_scripts() {
		...
		wp_enqueue_style(
			'godam-woo-featured-video-style',
			GODAM_WOO_URL . 'assets/build/css/godam-featured-video.css',
			array(),
			godam_woo_get_asset_version( GODAM_WOO_ASSETS_BUILD_PATH . 'css/godam-featured-video.css' )
		);

		wp_register_script(
			'godam-woo-featured-video-gallery',
			GODAM_WOO_URL . 'assets/build/js/wc-featured-video-gallery.min.js',
			array( 'jquery' ),
			godam_woo_get_asset_version( GODAM_WOO_ASSETS_BUILD_PATH . 'js/wc-featured-video-gallery.min.js' ),
			true
		);
		...
		wp_enqueue_script( 'godam-woo-featured-video-gallery' );

		do_action( 'rtgodam_featured_gallery_after_register_frontend_scripts' );
	}

	/**
	 * AJAX handler for generating gallery thumbnail HTML.
	 */
	public function handle_get_gallery_thumbnail() {
		...
		if ( ! $attachment_id || ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( __( 'Invalid attachment.', 'godam-woo' ) );
		}
	...
	/**
	 * Generate thumbnail HTML for a gallery item.
	 *
	 * @param int  $attachment_id Attachment ID.
	 * @param bool $is_video      Whether it's a video.
	 * @return string
	 */
	private function get_thumbnail_html( $attachment_id, $is_video ) {
		...
		$html .= '<ul class="actions"><li><a href="#" class="delete tips" data-tip="' . esc_attr__( 'Delete image', 'godam-woo' ) . '">' . esc_html__( 'Delete', 'godam-woo' ) . '</a></li></ul>';
		$html .= '</li>';
	...
	/**
	 * Save gallery images/videos on product save.
	 *
	...
	public function save_product_gallery_with_videos( $post_id ) {
		...
		if ( isset( $_POST['product_image_gallery'] ) ) {
			$raw_gallery = sanitize_text_field( wp_unslash( $_POST['product_image_gallery'] ) );
	...
	/**
	 * Replace the default WC gallery metabox.
	 */
	...
	/**
	 * Output the custom gallery metabox.
	 *
	...
	public function output_product_gallery_metabox( $post ) {
			...
			<ul class="product_images">
				...
				foreach ( $gallery_ids as $attachment_id ) {
					...
					if ( $is_video ) {
						...
						echo '<img src="' . esc_url( $src ) . '" class="attachment-thumbnail size-thumbnail" width="150" height="150" alt="" loading="lazy" decoding="async" srcset="' . esc_url( $src ) . ' 150w, ' . esc_url( $src ) . ' 300w, ' . esc_url( $src ) . ' 100w" sizes="auto, (max-width: 150px) 100vw, 150px" />';
					...
					if ( ! ( $is_video && ! $godam_has_valid_api_key ) ) {
						echo '<li><a href="#" class="delete tips" data-tip="' . esc_attr__( 'Delete image', 'godam-woo' ) . '">' . esc_html__( 'Delete', 'godam-woo' ) . '</a></li>';
					}
					echo '</ul></li>';
				}
				?>
		...
		<p class="add_product_images hide-if-no-js" id="godam-featured-gallery">
			<a href="#" data-choose="<?php esc_attr_e( 'Add images and videos to product gallery', 'godam-woo' ); ?>"
				data-update="<?php esc_attr_e( 'Add to gallery', 'godam-woo' ); ?>"
				data-delete="<?php esc_attr_e( 'Delete', 'godam-woo' ); ?>"
				data-text="<?php esc_attr_e( 'Delete', 'godam-woo' ); ?>">
				<?php esc_html_e( 'Add product gallery images and videos', 'godam-woo' ); ?>
			</a>
		</p>
		<?php
	}
	...
	/**
	 * Filter product gallery HTML for video items.
	 *
	...
	public function filter_single_product_image_html( $html, $attachment_id ) {
		...
		if ( $is_video ) {
			...
			$video_url = add_query_arg( $query_args, home_url( '/' ) );
